...
@Desc    :   Simple predictor script for bounding box PyTorch models.
'''
...
parser = argparse.ArgumentParser(
    prog="Datature Bounding Box PyTorch Predictor",
    description="Predictor to Predict Bounding Box Torch Model.")
...
def main():
    ...
    for each_image in glob.glob(os.path.join(input_path, "*")):
        ...
        detections_output = model(torch.Tensor(input_image))

        ## Filter detections
        detections_output = detections_output[0].detach()
        slicer = detections_output[:, -1]
        output = detections_output[:, :6][np.where(slicer != 0)]
        output = torch.Tensor(output)
        scores = output[:, 4]
        output = output[np.where(scores > threshold)]
        classes = output[:, 5]
        output = output[np.where(classes != 0)]
        output = torch.Tensor(output)

        ## Postprocess detections
        bboxes = output[:, :4].numpy()
